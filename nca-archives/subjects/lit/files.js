...
const ninthData = [
    `{
        ...
        "description": "A practice worksheet for the upcoming test",
        "isText": false
    }`,
    `{
...
class DataPiece {
    constructor(name, date, url, desc) {
        this.name = name;
        this.date = date;
        this.url = url;
...
function queryData(filterDate, filterName, filterGrade) {
    ...
    for (let x = 0; x < data.length; x ++) {
        ...
        let dataPiece = new DataPiece(d.name, d.date, d.url, d.description);
        if (isFilteringName) {
...
function search() {
    ...
    results.forEach(element => {
        ...
        newElem.innerHTML = `<p class="query">` + element.date + `<br> <a href="` + element.url + `">`+ element.name + `</a> <br>` + element.description + `</p>`;
...
document.getElementById("search-name").addEventListener("input", (event) => {
  ...
  document.getElementById("search-date").addEventListener("input", (event) => {
    search();
  });